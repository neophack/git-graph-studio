...
| # | Task | Touches | Acceptance |
|---|---|---|---|
| 4.1 ✅ | Parser layer: `tree-sitter` + 8 grammars, each behind a Cargo feature (all on by default); `build.rs` compiles the grammars (validate Windows MSVC first) | new `symbols/parse.rs` | A def / ref query `.scm` per language. Landed 2026-09-17 with nine grammars (rust, python, go, javascript, typescript, java, c, cpp, c-sharp — tree-sitter 0.25, compiled by cargo's `cc`, no build.rs change); one embedded query per language covers defs, call sites and imports, and a feature-off language falls back to the outline scan (also the M4.4 fallback) |
