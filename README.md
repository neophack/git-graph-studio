...
A standalone desktop app that wraps the `git-graph-rs` engine (the `plugin/` submodule's
`native/core`) in a small VS Code-like shell: an Explorer file tree with git status colouring,
...
- `static/gitgraph/view.html` — the webview host page (loads `out.min.js` prepared from `plugin/media/`)
- `src-tauri/` — the Tauri 2 backend: filesystem commands, the graph message protocol
  (`cmd_graph.rs`, served by `git-graph-core` from `plugin/native/core` in-process), the SCM
  git-CLI write path, and the PTY
- `plugin/` — the `vscode-git-graph-rs` extension, a git submodule tracking its repository's
  `main` branch: the engine crate the app links, the webview assets it compiles
...
Prerequisites: Rust 1.82+, Node.js, and the plugin submodule compiled once:

```sh
git submodule update --init      # checks out plugin/ (the extension)
cd plugin && npm install && npm run compile && cd ..   # out/config.js and media/
npm install
...
`target/studio/cargo/release/bundle/` — NSIS exe + MSI on Windows, dmg on macOS,
deb/rpm/AppImage on Linux (`bundle.targets` is `"all"`; each platform builds what applies
to it). Icons, the webview assets and the default view config are generated by
`beforeBuildCommand`, so no separate step is needed.

## Cross-platform releases

CI ([`studio.yml`](.github/workflows/studio.yml)) builds the installers natively per platform —
Windows x64, Linux x64, macOS arm64, plus macOS x64 on a `full` run. Linux arm64 is not built
(no arm64 WebKitGTK on the hosted runners). To bump the app's own version between releases,
change it in `package.json`, `src-tauri/tauri.conf.json` and `src-tauri/Cargo.toml` together.
