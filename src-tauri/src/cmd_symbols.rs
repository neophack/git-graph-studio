...
impl IndexStatus {
	fn empty() -> IndexStatus {
		IndexStatus { state: IndexState::Empty, done: 0, total: 0, files: 0, symbols: 0 }
	}
}
...
pub enum SymbolIndexEvent {
	...
	Done { files: usize, symbols: usize, cancelled: bool },
}
...
impl SymbolIndex {
	...
	pub fn status(&self, root: &str) -> IndexStatus {
		self.progress.lock().unwrap().get(root).cloned().unwrap_or_else(IndexStatus::empty)
	}

	fn note(&self, root: &str, status: IndexStatus) {
		self.progress.lock().unwrap().insert(root.to_owned(), status);
	}
	...
	fn install(&self, root: &str, store: SymbolStore) {
		...
		self.stores.lock().unwrap().insert(root.to_owned(), Arc::new(Mutex::new(store)));
	...
	pub fn symbols_with_refs(&self, root: &str) -> Option<(Vec<WorkspaceSymbol>, HashMap<String, usize>)> {
	...
	pub fn index_threads(background: bool) -> usize {
		...
		if background { (available / 2).max(2) } else { available }
	}
	...
	fn build_on_blocking(
		...
		let report = |done: usize, total: usize| {
			self.note(root, IndexStatus { state: IndexState::Building, done, total, files: 0, symbols: 0 });
		...
		let built = match saved {
			Some(mut store) => match store.refresh_against_disk(&report, &is_cancelled) {
				...
				Err(_) => SymbolStore::build(root, threads, &report, &is_cancelled).ok_or_else(|| "cancelled".to_owned()),
			},
			None => SymbolStore::build(root, threads, &report, &is_cancelled).ok_or_else(|| "cancelled".to_owned()),
		};
		...
		let store = match built {
			...
			Err(error) => {
				if let Some(channel) = channel {
					let _ = channel.send(SymbolIndexEvent::Done { files: 0, symbols: 0, cancelled: true });
				}
				return Err(error);
			}
		};
		...
		if let Some(channel) = channel {
			let _ = channel.send(SymbolIndexEvent::Done { files: stats.files, symbols: stats.symbols, cancelled: false });
		}
		if let Some(app) = app {
			use tauri::Emitter;
			let _ = app.emit(SYMBOL_INDEX_EVENT, self.status(root));
	...
	pub fn apply_changes(self: &Arc<Self>, app: Option<tauri::AppHandle>, root: &str, paths: Vec<String>) {
		let index = Arc::clone(self);
		let root = root.to_owned();
		std::thread::spawn(move || {
			let Some(store) = index.store(&root) else { return };
			{
...
fn ready_status(stats: &BuildStats) -> IndexStatus {
	IndexStatus { state: IndexState::Ready, done: stats.files, total: stats.files, files: stats.files, symbols: stats.symbols }
}

/* ---------- The commands ---------- */

fn resolve_root(state: &AppState, repo: Option<String>) -> Result<String, String> {
	repo.or_else(|| state.first_repo()).ok_or_else(|| "No folder is open".to_owned())
}

/// The index state of the open folder: what the status bar shows.
#[tauri::command]
pub async fn symbols_status(state: State<'_, AppState>, repo: Option<String>) -> Result<IndexStatus, String> {
...
pub(crate) async fn references_for(state: &AppState, name: &str, repo: Option<String>) -> Result<Vec<FileMatches>, String> {
	let root = resolve_root(state, repo)?;
...
pub(crate) fn scan_references(root: &str, name: &str, narrow: Option<Vec<String>>) -> Result<Vec<FileMatches>, String> {
...
fn tree_from(symbols: Vec<WorkspaceSymbol>, refs_of: impl Fn(&str) -> usize) -> Vec<TreeFile> {
	...
	for symbol in symbols {
		...
		let entry = TreeSymbol { kind: symbol.kind, name: symbol.name, line: symbol.line, refs };
		match files.last_mut() {
			Some(file) if file.path == symbol.path => file.symbols.push(entry),
			_ => files.push(TreeFile { path: symbol.path, symbols: vec![entry] }),
		}
	}
	files
}
...
pub async fn symbol_tree(
	...
	if let Some((symbols, counts)) = state.symbol_index.symbols_with_refs(&root) {
		return Ok(tree_from(symbols, |name| counts.get(name).copied().unwrap_or(0)));
	}
...
mod tests {
	...
	fn seeded_index() -> (tempfile::TempDir, tempfile::TempDir, Arc<SymbolIndex>, String) {
	...
	fn build_publishes_status_and_answers_queries() {
		...
		assert_eq!(all.len(), 2);
		assert_eq!(index.files_containing(&root, "beta"), Some(vec!["a.rs".to_owned(), "b.rs".to_owned()]));
	...
	fn a_saved_index_resumes_without_rebuilding_untouched_files() {
		...
		for _ in 0..100 {
			if resumed.lookup(&root, "gamma").is_some_and(|hits| !hits.is_empty()) {
				landed = true;
				break;
			}
	...
	fn a_cancelled_rebuild_keeps_the_ready_store_and_remove_drops_it() {
		...
		for i in 0..2000 {
			write(big.path(), &format!("f{i:04}.rs"), "fn big() {}
");
		}
		...
		assert_eq!(index.status(&root).state, IndexState::Ready, "the other root's cancel did not clear it");
		assert_eq!(index.lookup(&root, "alpha").unwrap().len(), 1);
	...
	fn symbol_tree_groups_files_with_reference_counts() {
		...
		assert_eq!(tree.iter().map(|file| file.path.as_str()).collect::<Vec<_>>(), ["a.rs", "b.rs"]);
	...
	fn symbol_references_scan_whole_words() {
		...
		assert_eq!((files[0].matches[0].line, files[0].matches[0].column), (1, 14));
