    ...
    fn finish(self, format: &'static str, start_timestamp_s: Option<f64>) -> CanLogStats {
        let mut messages: Vec<CanIdStats> = self
            ...
            .map(|((channel, id, _), a)| CanIdStats {
                ...
                avg_cycle_s: if a.count > 1 {
                    (a.cycle_sum_ns / ((a.count - 1) as u128)) as f64 / 1e9
                } else {
                    0.0
                },
...
fn analyse_intervals(
    ...
    let points: Vec<IntervalPoint> = deltas
        ...
        .map(|(i, d)| IntervalPoint {
            t_s: times.get(i + 1).copied().unwrap_or(0) as f64 / 1e9,
            cycle_s: *d as f64 / 1e9,
            missed: *d > loss_threshold,
        })
        .collect();
    drop(times);
    ...
    if max_ns > 0 {
        ...
        for b in 0..bins {
            histogram.push(CycleBin {
                from_s: (b * width) as f64 / 1e9,
                to_s: ((b + 1) * width) as f64 / 1e9,
                count: 0,
            });
        }
        for &d in &deltas {
    ...
    let mean = if deltas.is_empty() {
        ...
        (sum / deltas.len() as u128) as f64 / 1e9
    };
...
fn systemtime_to_epoch(st: &[u16]) -> Option<f64> {
    ...
    Some(
        (days * 86_400 + st[4] as i64 * 3_600 + st[5] as i64 * 60 + st[6] as i64) as f64
            + st[7] as f64 / 1_000.0,
    )
}
...
fn walk_blf_progress<R: Read>(
    ...
    r.read_exact(&mut header)
        .map_err(|_| "not a BLF file (shorter than its header)".to_string())?;
    ...
    if header_size > 144 {
        ...
        r.read_exact(&mut pad)
            .map_err(|_| "truncated BLF header".to_string())?;
    }
    let start_timestamp_s = (0..8)
        .map(|i| u16_at(&header, 40 + i * 2))
        .collect::<Vec<_>>();
    ...
    while let Some(base) = next_base(&mut r, &mut bytes) {
        ...
        if object_type == OBJ_LOG_CONTAINER {
            ...
            if method == 0 {
                ...
                if flate2::read::ZlibDecoder::new(payload)
                    .take(MAX_BLF_UNCOMPRESSED as u64)
...
impl<W: Write + Seek> BlfWriter<W> {
    fn new(mut out: W, start_epoch: f64) -> Self {
        ...
        BlfWriter {
            ...
            start_epoch,
            written_bytes: 0,
            buffer: Vec::with_capacity(CONTAINER_BYTES + 256),
            objects: 0,
    ...
    while let Some(token) = tokens.next() {
        ...
        if let Some((m, rest_of)) = token.split_once('/') {
            if let (Ok(month), Ok(day), Ok(year)) = (
                m.parse::<i64>(),
                rest_of.split_once('/')?.0.parse::<i64>(),
                rest_of.split_once('/')?.1.parse::<i64>(),
            ) {
                let secs = parse_hms(&mut tokens)?;
...
fn walk_asc_line(line: &str, state: &mut AscState, sink: &mut dyn FrameSink) {
    ...
    if let Some(rest) = line
        .strip_prefix("base ")
        .or_else(|| line.strip_prefix("BASE "))
    {
    ...
    if second.eq_ignore_ascii_case("CANFD") {
        ...
        for token in tokens {
            match token.as_bytes().first().copied().unwrap_or(b' ') | 0x20 {
                ...
                b'n' if token.eq_ignore_ascii_case("NOBRS")
                    || token.eq_ignore_ascii_case("NOBR") =>
                {
                    brs = false
                }
                b'e' if token.eq_ignore_ascii_case("ESI") || token.eq_ignore_ascii_case("ES") => {
                    esi = true
                }
                _ if n < slots.len() => {
                    slots[n] = token;
        ...
        let agrees = |dlc_tok: &str, count_tok: &str| -> Option<(u8, usize)> {
            ...
            let code = parse_num(dlc_tok)
                .and_then(as_code)
                .or_else(|| dlc_tok.parse::<u64>().ok().and_then(as_code))?;
        ...
        let (flags, kind, dlc, count) = match matched {
            ...
            None => (
                0,
                0,
                rest.first().and_then(|t| parse_num(t)).unwrap_or(0) as u8,
                0,
            ),
        };
        if flags == 2 {
        ...
        if matched.is_some() {
            for (slot, token) in bytes
                .iter_mut()
                .zip(rest.iter().skip(flags + kind + 2).take(count))
            {
    ...
    if remaining
        .iter()
        .any(|t| t.eq_ignore_ascii_case("ErrorFrame"))
    {
        sink.frame(error_frame(t_ns, channel));
        return;
    }
    ...
    let (dlc, remote, has_data) = match kind_byte {
        b'd' => (
            remaining.get(2).and_then(|t| parse_num(t)).unwrap_or(0) as u8,
            false,
            true,
        ),
        b'r' => (
...
fn walk_asc_reader<R: std::io::BufRead>(
    mut reader: R,
    sink: &mut dyn FrameSink,
    on_chunk: &mut dyn FnMut(u64, u64),
) -> (Option<f64>, &'static str) {
    let mut state = AscState {
        hex_base: true,
        start_epoch: None,
    };
...
impl<W: Write> AscWriter<W> {
    fn new(mut out: W, start_epoch: f64) -> Self {
        ...
        let result = writeln!(
            ...
            "date {} {} {} {:02}:{:02}:{:02}.{:03} {}",
            weekdays[weekday],
            months[(date[1] as usize - 1).min(11)],
            date[3],
            date[4],
            date[5],
            date[6],
...
impl ProgressReporter {
    ...
    fn tick(&mut self, frames: u64, bytes: u64, force: bool) {
        ...
        let _ = channel.send(CanProgress {
            frames,
            bytes,
...
fn walk_source(
    ...
    if read == 4 && &signature == b"LOGG" {
        ...
        let result = walk_blf_progress(
            Cursor::new(signature.to_vec()).chain(file),
            sink,
            &mut on_chunk,
        )?;
...
#[tauri::command]
pub async fn can_log_stats(
    path: String,
    on_progress: tauri::ipc::Channel<CanProgress>,
) -> Result<CanLogStats, String> {
...
/// the bridge.
#[tauri::command]
pub async fn can_intervals(
    path: String,
    channel: u16,
    id: u32,
    extended: bool,
    on_progress: tauri::ipc::Channel<CanProgress>,
) -> Result<CanIntervals, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let mut reporter = ProgressReporter::new(on_progress, total_bytes_of(Path::new(&path)));
        let mut collector = IntervalCollector {
            channel,
            id,
            times: Vec::new(),
            extended,
        ...
        Ok(analyse_intervals(
            std::mem::take(&mut collector.times),
            channel,
            id,
            collector.extended,
            collector.fd,
...
pub async fn convert_can_log(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let mut converter = if format.eq_ignore_ascii_case("blf") {
            Converter {
                writer: WriterTarget::Blf(BlfWriter::new(&mut out, start_epoch)),
            }
        } else {
            Converter {
                writer: WriterTarget::Asc(AscWriter::new(&mut out, start_epoch)),
            }
        };
...
impl FrameStore {
    fn push(&mut self, frame: &RawFrame) {
        let len = if frame.remote || frame.error {
            0
        } else {
            frame.len as usize
        };
    ...
    fn refresh_filter(&mut self, spec: &CanFrameFilter) {
        if !matches!(&self.filter, Some(cached) if cached.spec == *spec) {
            self.filter = Some(FilteredIndex {
                spec: spec.clone(),
                matches: Vec::new(),
                applied: 0,
            });
    ...
    fn filtered_window(
        ...
        (start.min(end)..end)
            .map(|i| self.line(filter.matches[i] as usize))
            .collect()
    }
}
...
#[tauri::command]
pub async fn can_log_open(
    path: String,
    state: tauri::State<'_, CanLogState>,
) -> Result<CanOpenResult, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let mut sink = VecSink {
            doc: Arc::clone(&doc),
            staging: Vec::with_capacity(RAW_SINK_BATCH),
        };
...
#[tauri::command]
pub async fn can_log_frames(
    state: tauri::State<'_, CanLogState>,
    doc_id: u32,
    start: usize,
    end: usize,
    filter: Option<CanFrameFilter>,
) -> Result<Vec<CanFrameLine>, String> {
    let doc = state
        .get(doc_id)
        .ok_or_else(|| "this log view was closed".to_string())?;
    tauri::async_runtime::spawn_blocking(move || {
...
#[tauri::command]
pub async fn can_log_count(
    state: tauri::State<'_, CanLogState>,
    doc_id: u32,
    filter: Option<CanFrameFilter>,
) -> Result<CanLogCount, String> {
    let doc = state
        .get(doc_id)
        .ok_or_else(|| "this log view was closed".to_string())?;
    tauri::async_runtime::spawn_blocking(move || {
...
fn parse_can_query(query: &str) -> Result<CanQuery, String> {
    ...
    if !cleaned.chars().all(|c| c.is_ascii_hexdigit()) {
        return Err(format!(
            "'{query}' is not a hexadecimal query — ids and data bytes are written in hex"
        ));
    }
...
fn find_positions<'a>(
    frames: &FrameStore,
    walk: impl Iterator<Item = (usize, u32)> + 'a,
    query: &CanQuery,
) -> CanFindResult {
    let mut out = CanFindResult {
        positions: Vec::new(),
        total: 0,
        capped: false,
    };
...
impl FrameStore {
    ...
    fn matches_query(&self, index: usize, query: &CanQuery) -> bool {
        ...
        let payload = &self.bytes[start..self.ends[index] as usize];
        payload
            .windows(query.bytes.len())
            .any(|window| window == query.bytes)
    }
}
...
#[tauri::command]
pub async fn can_log_find(
    state: tauri::State<'_, CanLogState>,
    doc_id: u32,
    query: String,
    filter: Option<CanFrameFilter>,
) -> Result<CanFindResult, String> {
    let doc = state
        .get(doc_id)
        .ok_or_else(|| "this log view was closed".to_string())?;
    tauri::async_runtime::spawn_blocking(move || {
        ...
        Ok(match &filter {
            None => find_positions(
                &frames,
                (0..frames.len()).map(|index| (index, index as u32)),
                &needle,
            ),
            Some(spec) => {
                frames.refresh_filter(spec);
                ...
                find_positions(
                    ...
                    filter
                        ...
                        .iter()
                        .enumerate()
                        .map(|(position, &frame)| (frame as usize, position as u32)),
                    &needle,
                )
            }
        })
...
mod tests {
    ...
    fn blf_reads_canoe_style_object_padding() {
        ...
        for (i, size) in [45usize, 46, 47, 48, 47].into_iter().enumerate() {
            first.extend(can_message_of_size(
                i as u64 * 1_000_000,
                1,
                0x100 + i as u32,
                size,
    ...
    fn blf_writer_pads_the_way_canoe_and_python_can_do() {
        ...
        for (i, size) in [48usize, 45, 46, 47].into_iter().enumerate() {
            writer.push_object(&can_message_of_size(
                i as u64 * 1_000_000,
                1,
                0x100 + i as u32,
                size,
        ...
        let mut writer = BlfWriter::new(Cursor::new(Vec::new()), 0.0);
        for i in 0..3_000u64 {
            writer.frame(&RawFrame::data_frame(
                i * 1_000_000,
                1,
                0x100 + (i % 4) as u32,
                false,
        ...
        while at + 16 <= bytes.len() {
            assert_eq!(
                &bytes[at..at + 4],
                b"LOBJ",
                "a strict size%4 reader desynchronises at {}",
                at
            );
        ...
        assert!(
            odd_residues >= 1,
            "the fixture never exercised a padded container"
        );
    }
    ...
    fn blf_read_the_start_time() {
        ...
        assert!(
            (start - (days as f64 * 86_400.0 + 8.0 * 3_600.0 + 30.0 * 60.0 + 0.25)).abs() < 1e-6
        );
    }
...
";
        ...
        assert_eq!(a.len, 20);
        assert_eq!(
            a.data[..a.len as usize],
            [
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x8b,
                0x0a, 0x00, 0x04, 0xe1, 0x40, 0x8b
            ]
        );
        ...
        assert_eq!(
            d.data[..8],
            [0x00, 0xa0, 0xaa, 0xaa, 0xaa, 0xaa, 0xa9, 0xa8]
        );
    }
    ...
    fn sample_frames() -> Vec<RawFrame> {
        let mut frames = vec![
            RawFrame::data_frame(
                ...
                8,
                8,
                &[0x17, 0x03, 0x22, 0x01, 0xF1, 0x26, 0x08, 0x08],
                false,
            ),
            RawFrame::data_frame(
                10_000_000,
        ...
        let mut fd =
            RawFrame::data_frame(40_000_000, 2, 0x200, false, true, 9, 12, &[0u8; 64], false);
        fd.brs = true;
        frames.push(fd);
    ...
    fn intervals_report_the_cycle_and_jitter() {
        ...
        let times: Vec<u64> = (0..1000)
            .map(|i| i as u64 * 10_000_000 + (i % 2) as u64 * 1_000_000)
            .collect();
...
";
        ...
        let days = days_from_civil(2026, 9, 14);
        assert!(
            (state.start_epoch.unwrap()
                - (days as f64 * 86_400.0 + 8.0 * 3_600.0 + 30.0 * 60.0 + 0.25))
                .abs()
                < 1e-6
        );
    }
    ...
    fn asc_date_layouts() {
        ...
        assert!(
            (parse_asc_date("Mon Sep 14 08:30:00.250 2026").unwrap()
                - (days * 86_400.0 + 30_600.25))
                .abs()
                < 1e-6
        );
        assert!(
            (parse_asc_date("09/14/2026 9:30:00 pm").unwrap()
                - (days * 86_400.0 + 21.0 * 3_600.0 + 1_800.0))
                .abs()
                < 1e-6
        );
        assert!(
...
";
    ...
    fn raw_filter_dimensions_narrow_independently_and_together() {
        ...
        assert_eq!(
            ids(&mut store, &CanFrameFilter::default()),
            vec![0, 1, 2, 3, 4, 5, 6]
        );
    ...
    fn raw_filter_index_extends_with_the_walk_and_rebuilds_on_change() {
        ...
        let other = CanFrameFilter {
            id_ranges: vec![(0x300, 0x300)],
            ..Default::default()
        };
    ...
    fn many_unique_ids_stay_bounded() {
        ...
        assert_eq!(
            stats.total_frames,
            stats.channels.iter().map(|c| c.frames).sum::<u64>()
        );
    ...
    fn a_giant_asc_parses_bounded_and_quickly() {
        ...
        for i in 0..LINES {
            ...
            writeln!(
                writer,
                "   {:.*} 1  {:X}  Rx   d 8 17 03 22 01 F1 26 08 08",
                6,
                i as f64 * 0.01,
                id
            )
...
";
        ...
        let standard = stats
            .messages
            .iter()
            .find(|m| m.id == 0x100 && !m.extended)
            .expect("a standard-id row");
        let extended = stats
            .messages
            .iter()
            .find(|m| m.id == 0x100 && m.extended)
            .expect("an extended-id row");
...
";
    ...
    fn messages_truncated_counts_identifiers_not_frames() {
        ...
        assert_eq!(
            ...
            (MAX_TRACKED_IDS - MAX_REPORTED_MESSAGES) as u64
        );
    }
    ...
    fn blf_writer_flushes_fd_and_error_frames_too() {
        ...
        while pos + 32 <= bytes.len() {
            ...
            assert!(
                uncompressed <= CONTAINER_BYTES + 256,
                "a container holds {uncompressed} uncompressed bytes"
            );
            containers += 1;
        ...
        assert!(
            containers >= 2,
            "{containers} containers for {fd_count} FD frames"
        );
    ...
    fn converting_a_log_onto_itself_is_refused() {
        ...
        let result = tauri::async_runtime::block_on(convert_can_log(
        ...
        assert!(
            result.is_err(),
            "converting a log onto itself must fail, not truncate it"
        );
        assert_eq!(
        ...
        let frames = tauri::async_runtime::block_on(convert_can_log(
            path.to_string_lossy().into_owned(),
    ...
    fn a_failing_asc_write_surfaces_as_an_error() {
        let mut converter = Converter {
            writer: WriterTarget::Asc(AscWriter::new(FailingWriter, 0.0)),
        };
    ...
    fn asc_candoe_writes_the_id_before_the_direction() {
        ...
        assert_eq!(lines.len(), 3);
        assert_eq!(
            (lines[0].channel, lines[0].id, lines[0].dlc),
            (21, 0x161, 13)
        );
        assert_eq!(lines[0].data_hex.split(' ').count(), 32);
        ...
        assert_eq!(
            (lines[2].channel, lines[2].id, lines[2].dlc),
            (7, 0x68F, 15)
        );
        ...
        let row = stats
            .messages
            .iter()
            .find(|m| m.channel == 7 && m.id == 0x68F)
            .expect("0x68F on channel 7");
