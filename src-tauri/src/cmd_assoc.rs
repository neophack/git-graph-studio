...
pub fn assoc_list_defaults() -> Vec<AssocExt> {
	CATALOG
		.iter()
		.map(|(ext, mime, recommended)| AssocExt { ext: (*ext).to_string(), mime: (*mime).to_string(), recommended: *recommended })
		.collect()
}
...
pub fn assoc_apply(extensions: Vec<String>) -> Result<AssocResult, String> {
	...
	return windows_impl::apply(&selected).map(|()| AssocResult { message_key: "assoc.applied.windows".into(), detail: String::new() });
	#[cfg(target_os = "linux")]
	return apply_linux(&selected).map(|detail| AssocResult { message_key: "assoc.applied.linux".into(), detail });
	#[cfg(not(any(target_os = "windows", target_os = "linux")))]
	{
		let _ = selected;
		Ok(AssocResult { message_key: "assoc.applied.macos".into(), detail: String::new() })
	}
}
...
mod windows_impl {
	...
	fn create_key(parent: &RegKey, path: &str, default: &str) -> Result<RegKey, String> {
		let (key, _) = parent.create_subkey(path).map_err(|e| format!("registry create {path}: {e}"))?;
		if !default.is_empty() {
			key.set_value("", &default).map_err(|e| format!("registry default {path}: {e}"))?;
		}
		Ok(key)
	}
	...
	pub fn apply_extension(classes: &RegKey, ext: &str, on: bool) -> Result<(), String> {
		...
		if on {
			...
			progid_key.set_value("", &format!("{APP_NAME} {ext} file")).map_err(|e| format!("registry default {id}: {e}"))?;
			let icon = create_key(classes, &format!("{id}\\DefaultIcon"), "")?;
			icon.set_value("", &exe).map_err(|e| format!("registry DefaultIcon: {e}"))?;
			let command = create_key(classes, &format!("{id}\\shell\\open\\command"), "")?;
			command.set_value("", &format!("\"{exe}\" \"%1\"")).map_err(|e| format!("registry command: {e}"))?;
			let ext_key = classes
			...
			ext_key.set_value(&id, &"").map_err(|e| format!("registry OpenWithProgids: {e}"))?;
		} else {
	...
	pub fn apply(selected: &[String]) -> Result<(), String> {
		...
		capabilities.set_value("ApplicationName", &APP_NAME).map_err(|e| format!("registry ApplicationName: {e}"))?;
		capabilities
			.set_value("ApplicationDescription", &"Git Graph Studio - repositories, editors, viewers and the CAN Trace Analyzer")
		...
		for ext in selected {
			file_assocs.set_value(format!(".{ext}"), &progid(ext)).map_err(|e| format!("registry FileAssociations: {e}"))?;
		}
		let registered = create_key(hkcu, "Software\\RegisteredApplications", "")?;
		registered.set_value(APP_NAME, &CAPABILITIES_PATH).map_err(|e| format!("registry RegisteredApplications: {e}"))?;
		Ok(())
	}
}
...
mod linux_impl {
	...
	fn set_mime_defaults(config: &Path, selected: &[String], desktop_id: &str) -> Result<(), String> {
	...
	pub fn apply(selected: &[String], data: &Path, config: &Path) -> Result<String, String> {
		...
		let mimetypes: Vec<String> = selected.iter().map(|e| format!("application/x-ggs-{e}")).collect();
		let desktop = format!(
			"[Desktop Entry]\nType=Application\nName=Git Graph Studio\nExec={} %f\nTerminal=false\nNoDisplay=true\nMimeType={}\n",
			exe.to_string_lossy(),
			mimetypes.join(";")
		);
		std::fs::write(apps.join("ggs.desktop"), desktop).map_err(|e| format!("ggs.desktop: {e}"))?;
		...
		std::fs::write(mime_packages.join("ggs.xml"), package).map_err(|e| format!("ggs.xml: {e}"))?;

		set_mime_defaults(config, selected, "ggs.desktop")?;

		// Refresh the caches when the tools exist; a missing one is not an error.
		let _ = std::process::Command::new("update-mime-database").arg(mime_packages.parent().unwrap()).status();
...
fn apply_linux(selected: &[String]) -> Result<String, String> {
	...
	let data = std::env::var("XDG_DATA_HOME").map(std::path::PathBuf::from).unwrap_or_else(|_| std::path::Path::new(&home).join(".local/share"));
	let config = std::env::var("XDG_CONFIG_HOME").map(std::path::PathBuf::from).unwrap_or_else(|_| std::path::Path::new(&home).join(".config"));
...
mod tests {
	...
	fn catalogue_marks_the_recommended_extensions() {
		...
		let recommended: Vec<&str> = catalog.iter().filter(|e| e.recommended).map(|e| e.ext.as_str()).collect();
	...
	fn linux_registration_writes_all_three_files() {
		...
		let desktop = std::fs::read_to_string(data.path().join("applications/ggs.desktop")).unwrap();
	...
	fn linux_registration_preserves_foreign_mimeapps_entries() {
		...
		assert!(rewritten.contains("text/plain=other.desktop"), "foreign default kept: {rewritten}");
		assert!(rewritten.contains("application/x-ggs-blf=ggs.desktop"));
		assert!(rewritten.contains("[Added Associations]"));
		assert!(rewritten.contains("application/pdf=other.desktop"), "foreign section untouched: {rewritten}");
