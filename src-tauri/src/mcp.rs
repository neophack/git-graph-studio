...
impl McpServer {
	...
	fn with_index(root: &str, index: Arc<SymbolIndex>) -> McpServer {
		McpServer { root: root.to_owned(), index }
	}
	...
	pub fn handle_line(&self, line: &str) -> Option<String> {
		...
		let method = message.get("method").and_then(Value::as_str).unwrap_or_default().to_owned();
	...
	fn call(&self, params: Option<&Value>) -> Result<Value, (i64, String)> {
		...
		let args = params.and_then(|params| params.get("arguments")).cloned().unwrap_or_else(|| json!({}));
		let text = match name {
	...
	fn tool_lookup(&self, args: &Value) -> Result<String, (i64, String)> {
		...
		Ok(match defs.as_slice() {
			...
			defs => {
				...
				for def in defs {
					out.push_str(&format!("{n}{kind} {name} — {path}:{line}", n = '\n', kind = def.kind, path = def.path, line = def.line + 1));
				}
				out
			}
		})
	...
	fn tool_references(&self, args: &Value) -> Result<String, (i64, String)> {
		...
		if total == 0 {
			return Ok(format!("No whole-word occurrences of '{name}' in the workspace's code files."));
		}
		let mut out = format!("{total} occurrence(s) of '{name}':");
		for file in files {
			for hit in &file.matches {
				out.push_str(&format!("{n}{path}:{line}:{column}", n = '\n', path = file.path, line = hit.line, column = hit.column));
			}
		}
		Ok(out)
	}
	...
	fn tool_tree(&self, args: &Value) -> Result<String, (i64, String)> {
		let prefix = args.get("path").and_then(Value::as_str).unwrap_or_default().replace('\\', "/");
	...
	fn tool_search(&self, args: &Value) -> Result<String, (i64, String)> {
		...
		Ok(match hits.as_slice() {
			...
			hits => {
				...
				for hit in hits {
					out.push_str(&format!("{n}{kind} {hit_name} — {path}:{line}", n = '\n', kind = hit.kind, hit_name = hit.name, path = hit.path, line = hit.line + 1));
				}
				out
			}
		})
...
pub fn run(folder: &str) -> Result<i32, String> {
	...
	eprintln!("[mcp] git-graph-studio {} serving {}", env!("CARGO_PKG_VERSION"), server.root);
...
mod tests {
	...
	fn call(server: &McpServer, tool: &str, args: Value) -> String {
		...
		assert!(response.get("error").is_none(), "tool call failed: {response}");
	...
	fn broken_lines_and_unknown_methods_answer_protocol_errors() {
		let (_dir, server) = scratch_server();
		let parsed: Value = serde_json::from_str(&server.handle_line("{ not json").unwrap()).unwrap();
		assert_eq!(parsed["error"]["code"], -32700);
		let parsed: Value = serde_json::from_str(&server.handle_line(r#"{"jsonrpc":"2.0","id":2,"method":"resources/list"}"#).unwrap()).unwrap();
		assert_eq!(parsed["error"]["code"], -32601);
		let parsed: Value = serde_json::from_str(
			&server.handle_line(r#"{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"nope"}}"#).unwrap(),
		)
		.unwrap();
	...
	fn tools_list_names_the_catalogue() {
		...
		let names: Vec<&str> = response["result"]["tools"]
			...
			.collect();
		assert_eq!(names, ["symbol_lookup", "symbol_references", "symbol_tree", "search_symbols", "index_status"]);
	}
	...
	fn lookup_references_tree_search_and_status_answer_over_the_index() {
		...
		assert!(references.contains("other.rs:1:4") && references.contains("other.rs:1:14"), "{references}");
		...
		assert!(search.contains("alpha") && search.contains("src/lib.rs"), "{search}");
	...
	fn empty_answers_stay_helpful_text_not_errors() {
		let (_dir, server) = scratch_server();
		assert!(call(&server, "symbol_lookup", json!({ "name": "missing" })).contains("No symbol named"));
		assert!(call(&server, "symbol_references", json!({ "name": "missing" })).contains("No whole-word occurrences"));
		assert!(call(&server, "search_symbols", json!({ "query": "zzz" })).contains("No symbol name contains"));
	}
}
